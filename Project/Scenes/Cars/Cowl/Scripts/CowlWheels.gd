extends Node
## COWL wheels Copied from Fanta1000

@export var sleep_start = 0.10
@export var sleep_start_rear = 0.50
...
func val_sleep(target: VehicleWheel3D, sleep_value) -> float:
	...
	if (val > 0 and val < sleep_value) or handbrake_pressed:
			play_sleep(target, val)
...
func play_sleep(target: VehicleWheel3D, volume: float) -> void:
	...
	target_player.volume_db = volume * eleven
